...
typedef struct Circle {
    ...
    float measured_coefficients[3] = { 0.50, 0.50, 0.50 };   // 0.5 on all sides "0.5->(-/+)<-0.5" // i=1+1 * 3; // 0=x;1=y;2=z;   // actual size of the "sphere" = 2 in diameter on all sides
    float radius_measure = 0.5;	// (unit) half the width of a circle in the scale of formula: "(x-axis*0.50) + (-x-axis*0.50)" scale in measures
    float radius = measured_coefficients[0];    // essentially divided by 2...
    float diameters = measured_coefficients[0] + measured_coefficients[1];
    float center[3] = { 0.00, 0.00, 0.00 };	// x,y,z coordinates
    float diameter_measure = 2.0 * radius;	        // (unit) measures
    float integral_number_of_all_sides = 6.66 * 6.66 * 6.66; // total measures of the spherical object
